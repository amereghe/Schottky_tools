...
% There is also a header file to be parsed.
    ...
    if ( strcmpi(lType,"SIG") )
        ...
        myX=(myMin:myDelta:myMax-myDelta)';
    elseif (strcmpi(lType,"FFT") )
        % - min freq [Hz]
    ...
    %% checks
    if ( length(myX)~=nValues )
        error("...something wrong at parsing! Different number of values between XX (%d) and YY (%d)",length(myX),nValues);
    end

    %% bye bye
    XX=zeros(nValues,nSignals);
    for ii=1:nSignals
        XX(:,ii)=myX;
    end
    if ( nSignals==1 )
